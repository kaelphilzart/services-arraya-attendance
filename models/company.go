...
func (m CompanyModel) One(id string) (Company interType.Company, err error) {
	err = db.GetDB().SelectOne(&Company, "select * from sc_users.company c where c.id = $1 order by c.created_at desc", id)
	return Company, err
}

// All ...
func (m CompanyModel) All() (Company []interType.Company, err error) {
	qs := `select * from sc_users.company c order by c.created_at desc`
