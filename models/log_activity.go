...
func (m LogActivityModel) All(params *interType.ParamsLogActivityAll) (logActivity []interType.LogActivity, err error) {
	_, err = db.GetDB().Select(&logActivity, "select la.id, la.user_id, u.'name', la.type, la.detail, u.url_photo from sc_user.log_activity la left join sc_user.users u on u.id = la.user_id where la.user_id = $1 order by la.created_at desc", params.UserID)
